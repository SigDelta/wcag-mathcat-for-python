        ...
        if m.lastgroup == "break":
            ...
            if ch.isupper():
                if synthConfig["sayCapForCapitals"]:
                    out.append(_("cap"))  # capital letter prefix
                if use_pitch:
                    out.append(
                        PitchCommand(multiplier=int(synthConfig["capPitchChange"]))
                    )
...
class MathCATInteraction(mathPres.MathInteractionNVDAObject):
    ...
    def reportFocus(self):
        ...
        try:
            speech.speak(
                ConvertSSMLTextForNVDA(
                    libmathcat.GetSpokenText(), self.provider._language
                )
            )
        except Exception as e:
            log.error(e)
    ...
    def _wrapMathMLForClipBoard(self, text: str) -> str:
        ...
        mathml_with_ns = text.replace(" data-changed='added'", "").replace(
            " data-id-added='true'", ""
        )
        ...
        try:
            with winUser.openClipboard(mainFrame.Handle):
                ...
                self._setClipboardData(
                    self.CF_MathML, self._wrapMathMLForClipBoard(text)
                )
                self._setClipboardData(
                    self.CF_MathML_Presentation, self._wrapMathMLForClipBoard(text)
                )
...
class MathCAT(mathPres.MathPresentationProvider):
    ...
    def _setSpeechLanguage(self, mathMl):
        ...
        try:
            ...
            speech.speakMessage(
                _(
                    "MathCAT does not support language %s: see NVDA error log for details"
                    % lang
                )
            )
